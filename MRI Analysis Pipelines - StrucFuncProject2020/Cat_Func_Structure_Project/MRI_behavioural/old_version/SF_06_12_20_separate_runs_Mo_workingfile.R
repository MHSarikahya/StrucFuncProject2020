...
#### SELECT RUNS USING ANNA's SPECIFICATIONS
sub_rm <- arithmetic_r %>%
  filter(!(ExperimentName == "Arithmetic_Run2" & Subject == 5 & Session == 2))
sub_rm <- sub_rm %>%
  filter(!(ExperimentName == "Arithmetic_Run1" & Subject == 6 & Session == 2))
sub_rm <- sub_rm %>%
  filter(!(ExperimentName == "Arithmetic_Run2" & Subject == 6 & Session == 1))
sub_rm <- sub_rm %>%
  filter(!(ExperimentName == "Arithmetic_Run1" & Subject == 7 & Session == 1))
sub_rm <- sub_rm %>%
...
sub_rm <- sub_rm %>%
  filter(!(ExperimentName == "Arithmetic_Run1" & Subject == 17 & Session == 2))
sub_rm <- sub_rm %>%
  filter(!(ExperimentName == "Arithmetic_Run2" & Subject == 23 & Session == 1))
#sub_rm <- sub_rm %>%  #unsure if should run - check notes
#  filter(!(ExperimentName == "Arithmetic_Run2" & Subject == 29 & Session == 2))
sub_rm <- sub_rm %>%
...
#sub_rm <- sub_rm_dm %>%
#  filter(!(ExperimentName == "DotMatrixEASY_Run1" & Subject == 58 & Session == 2))   #need to decide if keeping Run 1 or 2 for these
#sub_rm <- sub_rm_dm %>%
#  filter(!(ExperimentName == "DotMatrixEASY_Run1" & Subject == 59 & Session == 2))
#vswm_r  <- sub_rm_dm %>%
#  filter(!(ExperimentName == "DotMatrixEASY_Run1" & Subject == 63 & Session == 2))
...
#matching_r <- sub_rm_m %>%
#  filter(!(ExperimentName == "Matching_Run1" & Subject == 53 & Session == 1))    #need to decide if keeping Run 1 or 2 for these
...
#################################################################
### FIGURE OUT HOW TO HANDLE ARITHMETIC SUBJECT 33 WITH TWO DATES
#################################################################
...
#Accuracy
a <- aggregate(arithmetic_r$Problem.ACC, by= list(arithmetic_r$Subject, arithmetic_r$Session), mean, na.rm=TRUE)
names(a) <- c("Subject", "Session", "arithmetic_acc_mean")
arithmetic_summary <- merge(arithmetic_summary, a)

a1 <- aggregate(arithmetic_1$Problem.ACC, by= list(arithmetic_1$Subject, arithmetic_1$Session), mean, na.rm=TRUE)
names(a1) <- c("Subject", "Session", "arithmetic1_acc_mean")
arithmetic_1_summary <- merge(arithmetic_1_summary, a1)

a2 <- aggregate(arithmetic_2$Problem.ACC, by= list(arithmetic_2$Subject, arithmetic_2$Session), mean, na.rm=TRUE)
names(a2) <- c("Subject", "Session", "arithmetic2_acc_mean")
arithmetic_2_summary <- merge(arithmetic_2_summary, a2)

#RT
b <- aggregate(arithmetic_correct$Problem.RT, by= list(arithmetic_correct$Subject, arithmetic_correct$Session), mean, na.rm=TRUE)
names(b) <- c("Subject", "Session", "arithmetic_RT_mean")
arithmetic_summary <- merge(arithmetic_summary, b)

b1 <- aggregate(arithmetic_1_correct$Problem.RT, by= list(arithmetic_1_correct$Subject, arithmetic_1_correct$Session), mean, na.rm=TRUE)
names(b1) <- c("Subject", "Session", "arithmetic1_RT_mean")
arithmetic_1_summary <- merge(arithmetic_1_summary, b1)

b2 <- aggregate(arithmetic_2_correct$Problem.RT, by= list(arithmetic_2_correct$Subject, arithmetic_2_correct$Session), mean, na.rm=TRUE)
names(b2) <- c("Subject", "Session", "arithmetic2_RT_mean")
arithmetic_2_summary <- merge(arithmetic_2_summary, b2)


#### sort by Subject
arithmetic_summary <- arithmetic_summary[with(arithmetic_summary, order(arithmetic_summary$Subject)), ]
arithmetic_1_summary <- arithmetic_1_summary[with(arithmetic_1_summary, order(arithmetic_1_summary$Subject)), ]
arithmetic_2_summary <- arithmetic_2_summary[with(arithmetic_2_summary, order(arithmetic_2_summary$Subject)), ]
