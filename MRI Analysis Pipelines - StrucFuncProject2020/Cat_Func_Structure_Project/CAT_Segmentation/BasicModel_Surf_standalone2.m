function BasicModel_Surf_standalone(sub_raw, xsub, targetdir, reproc)
...
for i=1:size(xsub,2)
    for j=1:size(Dirs,2)
        ...
        temp_dir=fullfile(targetdir,xsub{i},Dirs{j});
    ...
    %temp_list = fullfile(DATADIR,subjects{i},Dirs{j},temp_files); %will return a cell array directly; perhaps faster than next 4 lines
        ...
        for k=1:size(temp_files,2)
           temp_list{1} = [temp_list{1}; {fullfile(targetdir,xsub{i},Dirs{j},temp_files{1,k})}];
        end
...
for i = 1:size(xsub,2)
  for j =1:size(Dirs,2)
            ...
            c.dataDir=fullfile(targetdir,xsub{i},Dirs{j});
            c.outDir=fullfile(targetdir,xsub{i},Dirs{j});
            c.files=files{i,j}{1};
...
function [matlabbatch]= spmdesign(c)
            ...
            %move file to sub dir
            movefile('/Users/Mohammed/Documents/Derivatives/SPM.mat', fullfile(targetdir, [xsub]));
    %estimate design
      % matlabbatch = {};
