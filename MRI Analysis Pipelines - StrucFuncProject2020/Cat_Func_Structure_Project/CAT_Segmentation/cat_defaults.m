...
%%%%%%%%%%%%%%%%%%%%%%%%
if exist('cat','var'), clear global cat; end
clear global cat; 
global cat
...
cat.opts.accstr    = 1;                     % SPM preprocessing accuracy (CAT only!) - 0 very low accuracy (fast) .. 1 very high accuracy (slow); default = 0.5
...
cat.estwrite.nproc = 4;  %check if works
cat.extopts.nproc      = 4; %check if works
...
cat.extopts.send_info    = 1;     % send Matlab and CAT12 version to SBM server for internal statistics only. If you dont want to send this 
